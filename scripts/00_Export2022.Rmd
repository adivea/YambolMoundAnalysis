...
The purpose of this script is to take all data collected during Elhovo 2022 campaign (ELhovo and Straldzha), enrich them with distance and azimuth to the nearest governing village (zemlishte) so that Todor can submit them to AKB
...
AKB asks for distance and azimuth to the nearest cadastral village (in "zemlishte"). The tricky part is that the cadastral villages need not be the geographically nearest places, and so we first need to establish which zemlishte territory is each mound (and join their attributes on the basis of a spatial join) in so we can calculate the distance between the 'capital' of the territory and all the mounds in the village cadastre.
...
Create distances to zemlishte for all features in Straldzha and Elhovo. Beware! May contain spatial duplicates!!!
```{r}
features <- readRDS("../output_data/features_aggr_27Dec.rds")
...
StrElh_data_extended <- StrElh_data %>% 
  left_join(features, by = "TRAP") %>% 
  dplyr::select(- EKATTE.x ,- geometry, - distTown,- distBG) %>% 
  dplyr::rename(Zemlishte = Name_en.x,DistanceZemlishte = distToTown,DirectionToZemlishte = azimuth)
