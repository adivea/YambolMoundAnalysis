...
This script prepares the data for follow up analysis. It helps you load the FAIMS data from 2009-2018 and 2022 on burial mounds from Yambol as well as associated Yambol region vectors and elevation rasters. It uses the latter for mound data enrichment. Its outputs are:
...
Yam <- raster("E:/TRAP Workstation/Shared GIS/Satellite imagery/ASTER/DEM/ASTGTM_N42E026/ASTGTM_N42E026_dem.tif")
Tur <- raster("E:/TRAP Workstation/Shared GIS/Satellite imagery/ASTER/DEM/ASTGTM_N41E026/ASTGTM_N41E026_dem.tif")
...
Y_elev <- mosaic(Yam, Tur, fun = mean, tolerance = 1)
...
Y_Lelev32635 <- projectRaster(Y_elev, crs = 32635)

writeRaster(Y_Lelev32635, file= ", format="GTiff", overwrite = TRUE)
rm(Y_elev)
rm(Y_Lelev32635)
```
...
# If loading the large mosaiced raster ( "../output_data/YLelev32635.tif" or the unprojected Y_elev.tif)because you have mounds outside Yambol region, rename and crop it to a buffer around Yambol 

elev <- raster("../output_data/YLelev32635.tif")
plot(elev)
# elev <- crop(Y_Lelev32635, st_buffer(Y_region, 5000))
# elev <- mask(elev, st_buffer(Y_region, 5000))
# plot(elev)
# Y_elev <- crop(Y_elev32635, Y_region)
...
plot(mn22$geometry);plot(Y_flow, add =T); plot(mn22$geometry, add= TRUE)
  ...
  # Write to file
writeRaster(Y_aspslope, filename = "../output_data/Yaspslope35.tif", format="GTiff", overwrite = TRUE)
