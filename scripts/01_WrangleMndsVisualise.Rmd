...
## Load data
```{r -load-aggr-data}
features <- read_rds("../output_data/mnds_aggr.rds")
str(features)
#colnames(features)
```

## Add data necessary for AKB -- NEEDS COMPLETION
### add vilage cadasters in the Elhovo municipality
```{r AKB-data}
...
plot(Yam_mnds,cex = sqrt(Yam_mnds$HeightMax))
```

```{r plot-mnds-in-Y, eval = FALSE}
#par(mfrow=c(1,2))
raster::plot(Y_elev, xlim = c(420000,520000), ylim=c(4600000, 4800000));
raster::plot(Yam_mnds$geometry, add = T)
raster::plot(Y_elev, xlim = c(420000,520000), ylim=c(4600000, 4800000));
raster::plot(Yam_mnds$geometry, cex = sqrt(Yam_mnds$HeightMax), add =T)
```
...
library(mapview)
class(Yam_mnds)
mapview(Yam_mnds, cex="HeightMax")
```
