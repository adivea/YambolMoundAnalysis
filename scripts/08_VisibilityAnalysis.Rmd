...
## Parallelize over rasters with modelled vegetation
Rather than calculation LoS over bare earth model, let's model it over surface where random stands of trees and scrub has been modelled. We have two rasters produced in 08_VisibilityProm.R:

1. vegetation is modelled as random stands of trees of 10m height covering 50% of the extent of Yambol (randomly)
2. vegetation varing in height from 1-20m (normally distributed with peak at 10) modelled covering 50% of the extent of Yambol randomly

These rasters are overlaid over Yambol elevation and values summed, increasing the terrain in places by 10 - 20 m (producing Y_elev10 and Y_elev20grad respectively).

The hypothesis is that the vegetation will reduce the LoS among mounds that are at lower elevation differences or closer to one another.
...
### Intervisibility results in a model with 0-20m vegetation
...
```{r results-test}
# Check the results of the initial intervisibility run
result_table <- readRDS("../output_data/veg20grad_los.rds")
...
#### Link results to actual mound  in veg20 gradual

```{r link-TRAP-to-results-veg20}
result_table$TRAPorigin <- Yam_mnds$TRAP[result_table$i]
result_table$TRAPtarget <- Yam_mnds$TRAP[result_table$e]


# all the mounds visible from mound 9412 no. 333 (n = 409)
result_table %>% 
  filter(i == 333, visibility == 1) %>% 
  arrange(desc(e)) %>% 
  distinct(TRAPtarget) %>% 
...
# The new leader mound 9044 no. 38 (n = 291)
result_table %>% 
  filter(i == 38, visibility == 1) %>% 
  arrange(desc(e)) %>% 
  distinct(TRAPtarget) %>% 
...
saveRDS(result_table, "../output_data/veg20grad_los.rds")
# i and e are the same as TRAPorigin and TRAPtarget
```


### Intervisibility results in a model with 10m vegetation
...
# Check the results of the initial intervisibility run
result_table <- readRDS("../output_data/veg10los.rds")
result_table <- as_tibble(result_table)
...
### COmbatting edge effect
Calculate intervisibility for mounds among those 5 km to the border and the mapped mound and then add this result to the current intervisibility table. How many mounds outside Yambol can these mounds see?

```{r}
innerbuff <- st_buffer(Y_region, -5000)
plot(Y_region$geometry);plot(innerbuff, add = T)
#plot(st_intersection(Yam_mnds, innerbuff), add = T)
plot(st_difference(Yam_mnds, innerbuff), add = T)

edgemounds <- st_difference(Yam_mnds, innerbuff)
...
mnd_vis %>% 
  ...
  # show other mounds in the background as small dots
  mapview(Yam_mnds,cex = 0.1)+
  mapview(Yam_mnds %>% filter(prom2000mbuff>90 &prom250mbuff>90 ), cex = 10)
```
...
### Bring in prominence at greater radius
```{r}
prom2000 <- raster("../output_data/prom_clean/YT_prom2000m.tiff")
plot(prom2000)
Yam_mnds <- Yam_mnds %>% 
...
library(raster)
extract
Yam_mnds$prom2000mbuff <- raster::extract(prom2000, st_coordinates(Yam_mnds))

Yam_mnds %>% 
  filter(prom2000mbuff > 95) %>% 
  mapview(zcol = "prom2000mbuff") # 9412 not present!

Yam_mnds %>% 
  filter(prom250mbuff > 95) %>% 
  mapview(zcol = "prom250mbuff")

# Three mounds are in areas of prominence over 95% in both 250 and 2000m radius 
Yam_mnds %>% 
  filter(prom250mbuff > 95) %>% 
  filter(prom2000mbuff > 95) %>% 
  mapview()
  
# Twelve are in areas of 90% prominence
Yam_mnds %>% 
  filter(prom250mbuff > 90) %>% 
  filter(prom2000mbuff > 90) %>% 
  mapview()
...
Yam_mnds %>% 
  filter(prom250mbuff > 90) %>% 
  filter(prom2000mbuff > 90) %>%
  select(TRAP:Type, HeightMax, prom250mbuff, prom2000mbuff) %>% 
  arrange(desc(prom2000mbuff))
```
