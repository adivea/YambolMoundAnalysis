...
### Visibility lines for 9412 and mounds
...
```{r}
dist2km <- st_buffer(Yam_mnds %>% filter(TRAP== 9412), 2500)
dist5km <- st_buffer(Yam_mnds %>% filter(TRAP== 9412), 5000)
dist10km <- st_buffer(Yam_mnds %>% filter(TRAP== 9412), 10000)


mapview(lines_sm, cex = 0.1)+ 
  mapview(Yam_mnds %>% filter(TRAP %in% mostvisiblemnd)) +
  mapview(dist2km) + mapview(dist5km) + mapview(dist10km)
```


### Intervisibility with settlements
Let's test for visibility between mounds and major settlements as well as the river/road - eg. Kabyle, Dadopara.
