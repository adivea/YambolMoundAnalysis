...
Lozyanska - UTM 35 T 468101 4679119 - is 6003
...
The following map will show the mounds within the Yambol dataset that have prominence higher than the mean of 70% (258 features) and whose topographic ruggedness is over the mean of 1.6 (52). The lighter the point color the higher their prominence and the more probable their Bronze Age origin 
```{r}
library(mapview)
library(leafsync)
tri <- Yam_mnds %>% 
  filter( prom250mbuff > 70 & Type == "Burial Mound" & TRI >1.6) %>% 
  mapview(zcol = "prom250mbuff")
notri <- Yam_mnds %>% 
  filter(prom250mbuff > 70 & Type == "Burial Mound") %>% 
  mapview(zcol = "prom250mbuff")
BA <- mapview(BApoints)
sync(tri,notri, BA, ncol=3)
```
## Predict BA mounds on basis of Landuse and size
Over 3 m high and over 70% and 1.6 TRI results in 6 features total
```{r}
withTRI <- Yam_mnds %>% 
  filter(HeightMax > 3 & 
        #   elevAster > 150 &
...
Over 3 m high and 70%+ prominence results in 51 features.
```{r}
library(mapview)
library(leafsync)
noTRI <- Yam_mnds %>% 
