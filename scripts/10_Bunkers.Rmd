...
This script also processes Angel's digitized Map Mounds - this section needs to eventually be moved to a separate script!!

To DOES: 
 - national boundaries for a good inset map
...
```
### Add and clean map mounds Angel digitized in June 2024
this concerns mounds north of Yambol within 25 km buffer 
```{r}
mapmnds_a <- read_sf("../data/angel_mapmnds/MoundsAngel.shp")


# fix angels' mounds IDs and Symbols
mapview(mapmnds_a, cex = 3) + mapview(mapmnds, cex =1)
mapmnds_a$Id <- 400000:400284
mapmnds_a$createdBy <- "Angel Grigorov"
mapmnds_a$createdAt <- "2024-06-06"
mapmnds_a <- mapmnds_a %>% 
...
mapmnds_a <- mapmnds_a %>% 
  mutate(MapSymbol = case_when(is.na(MpSymbol) ~ "Sunburst",
                               !is.na(MpSymbol) ~ MpSymbol)) %>%
  dplyr::select(-MpSymbol) %>% 
  mutate(Notes = "digitized in 2024")
#st_write(mapmnds_a, "../data/MapMoundsA32635.geojson")


# merge Angel's with main mapped mounds
mapmnds <- mapmnds %>% 
...
### Explore the military features a bit

Most seem collected in 2018 and 2022 seasons, even though we have seen emplacements since 2009 (on Boyanovo quarry south slope)
```{r explore-mil}
tail(Yam_mil)
Yam_mil <- Yam_mil[-79,]
...
```{r get-map-mnd}
table(mapmnds$MapSymbol)  # 'circle' seems the best think to search on with ca 4289records

mapmnds <- mapmnds %>% 
  filter(grepl("circle|Sunburst",MapSymbol))
...
tm_shape(Yam_bunkers, bbox = st_bbox(st_buffer(st_geometry(Yam_bunkers[1,]), 20000))) + 
  tm_dots(shape = "Feature type", #6 and 26 are downward triangles
            alpha= 0.4,
            title = "Type of \nmilitary features",
             size = "HeightMax", # was 0.3
          col = "yellow") +
   tm_text(text = "TRAP", size = 1)+  
tm_shape(mapmnds) +
  tm_dots(size = 0.01)+
tm_shape(Y_region) +
 tm_borders(col = "grey",
...
```
### Export data for Angel's viewsheds
```{r}
# filter bunkers
features %>% 
  filter(TRAP %in% c(8595, 9845, 8619,8515, 8707, 8535, 8244, 8271, 8798, 8230, 
                     9836, 9841, 9721, 8531, 8709,8711, 8278, 8798)) %>% # pull(TRAP) %>% sort()
  st_write("D:/Daaaaaataaaa/bunkers.shp")

group_by(Type) %>% 
  tally()
st_write(Yam_mnds, "../data/Yam_mnds.shp")
st_write(features, "../data/Yam_features.shp")
```
