...
## Visualize military installations on the Cold-War border
...
```
## Map mounds 
are good for the background, but now the file contains all kinds of benchmarks. Let's filter to the sunburst, brown sun-like symbol, which in 99% cases leads to a urial mound.
...
mapmnds
Y_region
plot(mapmnds$geometry); plot(Y_region$geometry,add = T)
```
...
overview_yammnds
```
### Local typology - elaboration
```{r}
library(tmap)
tm_shape(Yam_bunkers, bbox = st_bbox(st_buffer(st_geometry(Yam_bunkers[1,]), 20000))) + 
  tm_dots( shape = "CentralFeature", #6 and 26 are downward triangles
            alpha= 0.4,
            title = "Type of \nmilitary features",
             size = 0.3) +  
tm_shape(Y_region) +
...
color_type <- rainbow(length(unique(Yam_bunkers$FD)))
library(tmap)
tm_shape(Yam_bunkers, bbox = st_bbox(st_buffer(st_geometry(Yam_bunkers[44,]), 1100))) + 
  tm_dots( col = "FD",
           shape = "CentralFeature",
          palette = color_type,
...
tm_shape(Y_region) +
 tm_borders(col = "grey",
  ...
  tm_scale_bar(position = c("left", "bottom"),
               breaks = c(0,0.5,1), 
               text.size = 0.75)  +
  tm_layout(title = "B", legend.outside = FALSE)
```
Try facets on bearing
```{r}
...
bg_inset <- plot(e)
plot(BG$geometry, col = "white", border = "black", add = T); 
plot(Y_region$geometry,col = "darkgrey", border= "darkgrey", add = T)
plot(GR$geometry, add = T)
plot(TR$geometry, add = T)
```

Prepare to print with the inset NOT READY
```{r eval = FALSE}
library(tmaptools)
library(grid)
...
# print insets
print(bg_inset, vp=viewport(x= 0, y= 0, width= 0.3, height= 0.3))
